  ...
  await client.guilds.get("538808292722475019").members.forEach(async m => {
    await msg.guild.members.forEach(async m1 => {
      if (m.id === m1.id && m.roles.has("545413502668636191")) {
        ...
        if (type === "movie") {
          await fetch(
            `${config.OMBI_FULLURL}/ombi/api/v1/Search/movie/${query}`,
            {
              method: "get",
              headers: {
            ...
            .then(async data => {
              ...
              for (let i = 0; i < data.length; i++) {
                ...
                if (data[i].firstAired)
                  fieldContent += ` (${data[i].firstAired})`;
                fieldContent += "\n";
              }
          ...
          await fetch(
            ...
            .then(async json => {
              ...
              if (json.length <= 1) {
                // output data into chat and request the tv show
                return utils
                  .outputMusic(msg, json[selection])
                  .catch(async error => {
                    await msg.channel.send("[ERROR] Cancelled command.");
                  });
              }
              ...
              for (let i = 0; i < json.length; i++) {
                ...
                if (json[i].firstAired)
                  fieldContent += ` (${json[i].firstAired})`;
                fieldContent += "\n";
              }
